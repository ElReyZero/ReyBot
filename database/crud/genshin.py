    ...
    with Session(bind=engine) as session:
        for wishType in ["Character Event", "Weapon Event", "Standard"]:
            ...
            for _, row in df.iterrows():
                wish = Wish(type=row['Type'], name=row['Name'], time=row['Time'], stars=row['⭐'], pity=row['Pity'], roll_no=row['#Roll'], group=row['Group'], banner=row['Banner'], wish_type=wishType)
    ...
    with Session(bind=engine) as session:
        for char in chars:
            if not task:
                ...
                log.info(f"SQLAlchemy - Character data:")
            ...
            if char_entry:
                ...
                session.commit()
                log.info("SQLAlchemy - Character already exists in database. Updating") if not task else ""
                log.info(f"SQLAlchemy - Updated {char.name} successfully") if not task else ""
            else:
                ...
                char_entry = Character(character_id=char.id, name=char.name, element=char.element, rarity=char.rarity, icon=charIcon, collab=char.collab, level=char.level, friendship=char.friendship, constellation_level=char.constellation, weapon=char_weapon, constellations=constellation_list)
